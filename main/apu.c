...
static CPU_WRITE_HANDLER(frame_counter_write_handler)
{
	...
	if (apu->odd_cycle)
		cycles += 2 * emu->cpu_clock_divider;
	else
		cycles += 3 * emu->cpu_clock_divider;

	printf("value: %x cycles: %d, next_frame_step: %d, diff: %d\n", value,
	       cycles, apu->next_frame_step, apu->next_frame_step - cycles);
...
void apu_reset(struct apu_state *apu, int hard)
{
	...
	apu->next_frame_step = apu->frame_step_delay *
	    apu->emu->cpu_clock_divider;

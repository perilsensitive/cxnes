...
struct range_list *add_range(struct range_list **ranges, int off, int len)
{
	...
	if (!new_range) {
		fprintf(stderr, "add_range: malloc() failed\n");
		return NULL;
	}
		    ...
		    off_t target_offset, uint8_t *patch, size_t patch_size)
{
	...
	if (memcmp(patch, "UPS1", 4) != 0) {
		fprintf(stderr, "patch_apply_ups: invalid UPS header\n");
		return -1;
	}
	...
	if (expected_data_size != *data_size - data_offset) {
		fprintf(stderr, "patch_apply_ups: expected data size (%zu) "
	...
	if (!real_target) {
		fprintf(stderr, "patch_apply_ups: malloc() failed\n");
		return -1;
	}
		    ...
		    off_t target_offset, uint8_t *patch, size_t patch_size)
{
	...
	if (memcmp(patch, "BPS1", 4) != 0) {
		fprintf(stderr, "patch_apply_bps: invalid BPS header\n");
		return -1;
	}
	...
	if (expected_data_size != *data_size - data_offset) {
		fprintf(stderr, "patch_apply_bps: expected data size (%zu) "
			"does not equal actual data size (%zu)\n",
			expected_data_size, *data_size);
		return -1;
	}
	...
	real_target = malloc(target_size + target_offset);
	if (!real_target) {
		fprintf(stderr, "patch_apply_bps: malloc() failed\n");
		return -1;
	}
	target = real_target + target_offset;
		    ...
		    struct range_list **ranges)
{
	...
	if (memcmp(patch, "PATCH", 5) != 0) {
		fprintf(stderr, "patch_apply_ips: invalid IPS header\n");
		return -1;
	} else if ((memcmp(patch + patch_size - 3, "EOF", 3) != 0) &&
		   (memcmp(patch + patch_size - 6, "EOF", 3) != 0)) {
		fprintf(stderr, "patch_apply_ips: invalid IPS footer\n");
		return -1;
	}
